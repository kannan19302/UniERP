> Historical, non-authoritative copy of `provider-admin/ARCHITECTURE.md`. Preserved during the 14-root documentation consolidation on 2026-09-24. Current architecture is owned by the accepted ADRs and `platform/docs/platforms/`; current repository rules are in its root `AGENTS.md`.

# Architecture Specification: UniERP Provider Admin OS (Control Plane) (`provider-admin-os`)
...
```mermaid
graph LR
  Callers["Allowed Inbound Callers<br/>Platform operators, SREs, super-administrators"] --> Repo["<b>provider-admin-os (L4)</b><br/>UniERP Provider Admin OS (Control Plane)"]
  ...
  Forbidden["Strictly Forbidden<br/>Direct database ORM, L2 internals, Bypassing ControlPlaneGuard"] -.-x Repo
...
### Boundary Contract
- **Allowed Inbound Consumers**: Platform operators, SREs, super-administrators
...
- **Primary Interface**: `@kannan19302/console`
...
- **Project Skill**: [`.agents/skills/provider-admin-standards/SKILL.md`](.agents/skills/provider-admin-standards/SKILL.md)
