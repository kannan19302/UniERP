> **Repository evidence:** Relocated from `unierp-platform/README.md`. This file may describe local setup, contribution or history but cannot override the owning product/platform suite.
...
> monorepo. `ROADMAP.md` (legacy reference: `ROADMAP.md`) says exactly what is and is not finished,
> and `GOVERNANCE.md` (legacy reference: `GOVERNANCE.md`) says who maintains it. Please read both
...
| **Contributing** | `CONTRIBUTING.md` (legacy reference: `CONTRIBUTING.md`) — it starts by telling you which of the repositories below your change belongs in |
| **Stuck, or unsure where to ask** | `SUPPORT.md` (legacy reference: `SUPPORT.md`) |
| **Wondering how finished this is** | `ROADMAP.md` (legacy reference: `ROADMAP.md`) — read this before you deploy it |
| **Understanding the design** | `ARCHITECTURE.md` (legacy reference: `ARCHITECTURE.md`) here, then [`PLATFORM_ARCHITECTURE.md`](https://github.com/kannan19302/ERPSys/blob/main/docs/PLATFORM_ARCHITECTURE.md) for the full specification |
...
standalone — see § 14.1 for what unblocks that, and `ROADMAP.md` (legacy reference: `ROADMAP.md`)
...
GNU Affero General Public License v3.0 (legacy reference: `LICENSE`). Self-hostable in full, with
