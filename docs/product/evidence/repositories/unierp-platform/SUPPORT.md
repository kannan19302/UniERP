> **Repository evidence:** Relocated from `unierp-platform/SUPPORT.md`. This file may describe local setup, contribution or history but cannot override the owning product/platform suite.
...
| You have | Go to |
| :------- | :---- |
| A **security vulnerability** | [Private advisory](https://github.com/kannan19302/UniERP/security/advisories/new). Never a public issue — see `SECURITY.md` (legacy reference: `SECURITY.md`) |
| A **bug in specific code** | An issue on the repository that owns it, from the repository map (legacy reference: `README.md#repository-map`) |
...
| A **feature idea** | Discussions first, after checking `ROADMAP.md` (legacy reference: `ROADMAP.md`). It becomes an issue once it is concrete |
...
cannot yet install standalone (legacy reference: `ROADMAP.md#where-the-project-actually-is`), so
...
| | |
| :-- | :-- |
| How the system is put together | `ARCHITECTURE.md` (legacy reference: `ARCHITECTURE.md`) |
...
| Running it locally | Run it locally (legacy reference: `README.md#run-it-locally`) |
| What is planned, and what is deliberately not | `ROADMAP.md` (legacy reference: `ROADMAP.md`) |
| Who decides, and how to get commit rights | `GOVERNANCE.md` (legacy reference: `GOVERNANCE.md`) |
