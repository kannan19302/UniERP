> **Repository evidence:** Relocated from `unierp-platform/CONTRIBUTING.md`. This file may describe local setup, contribution or history but cannot override the owning product/platform suite.
...
| API, web, data, kernel, sandbox, SDK — any actual code | The repository that owns it, from the repository map (legacy reference: `README.md#repository-map`) |
...
locally (legacy reference: `README.md#run-it-locally`) that did not work on your machine. That last
...
person — see `GOVERNANCE.md` (legacy reference: `GOVERNANCE.md`) for what that means for your
...
Contributions are accepted under AGPL-3.0 (legacy reference: `LICENSE`), the licence the project
ships under. By opening a pull request you agree your contribution may be
distributed under it. There is no separate CLA.

Participation is governed by the Code of Conduct (legacy reference: `CODE_OF_CONDUCT.md`).
