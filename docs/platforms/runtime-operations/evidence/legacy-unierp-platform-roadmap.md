> **Migration status:** Legacy roadmap retained for history; it is not an implementation plan or current commitment.

# Roadmap

**No dates.** UniERP is maintained by one person (see `GOVERNANCE.md` (legacy reference: `GOVERNANCE.md`)),
...
- The **architecture** is the layered polyrepo described in the repository
  map (legacy reference: `README.md#repository-map`). Those repositories exist and carry real code.
