...
The card must not ask the user to choose a tenant/provider sign-in scope or enter an organization slug. The
server derives the authority realm from the validated OIDC relying-party destination and resolves an unambiguous
tenant membership from the submitted email. Browser-submitted scope or slug fields are not trusted. Multiple
memberships are resolved only after authentication; the service never guesses a tenant or provider realm from an
email domain.

Desktop and mobile presentations use a compact single-column card with concise copy, 40–44 px controls, visible
keyboard focus and reduced-motion support. Compactness must not remove persistent labels, legal/version context,
error recovery, touch-target size or assistive-technology semantics.
